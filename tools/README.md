...
| `voice_leak_rate.py` | **LIVE, diagnostic** (2026-08-30): how often does she ANALYSE him instead of TALKING to him? Counts recorded assistant turns whose first sentence is about him in the third person or is a self-instruction — the shape that reaches the room AND her journal. A TOOL and not a gate on purpose: the rate is a property of the model and the evening's material, so a gate over it would go red on a quiet night. Ten-day baseline in its docstring (~13%, no trend) so a later reader can tell drift from noise | `docs/OFF-BY-DEFAULT.md` §18's arming condition ("re-measure afterwards"), AGENTS.md §4 trap 8 |
