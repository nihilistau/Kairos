...
### 0. `SP_HOUSE_HANDS` — she can turn his lights on and off *(recorded 2026-08-27; **ARMED, on-request, 2026-08-27 — the operator's call, "arm it"**)*

**STATUS: ON, on the live profile (`hands = true`), on-request only.** The allowlist lives
in `var/house-allow.json` (his file, five entities, spoken-name → id), the actable domains
are a closed set in code (`light`, `fan` — never `switch.*`), and acting while he is away
is refused on the presence clock. AUTONOMOUS action remains a separate, later, off-by-
default arming with its own row when it is built. The paragraphs below are the row as it
was written the day before arming, kept because the reasoning is the boundary's design:
...
| **On request only** | The operator's call: *"on-request only with autonomous action as a separate, later, off-by-default arming."* Enforced on the same `_seconds_since_he_spoke` clock the room veto reads — not a second copy of it, which this gate refuses. |
...
| **Gate** | `harness_tests/g_mcp_trust.py` §7b — 59/59 |
...
| `SP_ENGINE_MARGIN_APPROX` (`[engine].margin_approx`) | **OFF by default — an approximation, said so.** Without the daemon's `eot_margin` the kairos CONTINUE/EXPAND impulses are dark (REMIND / SOLO / MUSE / CHECK_IN live). On, a `finish_reason == "length"` reads as "cut off" — crude, no magnitude, so the scheduler gives it the SHALLOWEST cut-off the policy recognises (`continue_margin - 1.0`) and therefore the longest continuation delay: a guess should not also claim urgency. **Until 2026-09-02 the backend set margin `0.0` and this knob did nothing whatsoever** — 0.0 is above `continue_margin` (-18.50), so it read as a comfortably finished turn. Anyone who turned it on got silence and no way to tell. G-CONTINUATION-MARGIN §2. | A measured look at how often a length-stop on the chosen endpoint is a real mid-thought cut; until then the honest default is dark. |
...
### 12b. `kairos.continue_enabled` — she finishes a cut-off thought *(recorded 2026-09-02)*

| Knob | Why off / state | What would arm it |
|---|---|---|
| `kairos.continue_enabled` | **OFF by default, by the operator's decision (2026-09-02), and MEASURED before it was taken:** 758 continuations withheld and the last `SPOKE (continue)` on 2026-08-20 08:12, i.e. the lane had produced nothing in thirteen days. His words: *"i'm unsure about continue, lets default it to off. it always seems to create problems and slow downs."* Gates the whole `eot_margin` block, so it takes **EXPAND with it** — both lanes read the same signal and disabling one would ship half a decision. **It had already been dark for thirteen days by accident:** arming `quiet_after_him_s` to 300 on 2026-08-20 made the lane unreachable, because a continuation is decided seconds after his turn and that gate wants five minutes of his silence. 758 withheld; last `SPOKE (continue)` 2026-08-20 08:12. So this changes nothing today — it turns an accident into a decision, which is the point: a lane that is off by side effect comes back by side effect. | Set `kairos.continue_enabled = true`. **And check the second gate**: with `quiet_after_him_s` at 300 the lane is still unreachable, so arming this alone will look like it did nothing. The open ledger card `quiet-after-him-vs-continue` is that decision and it is his. Verified reachable 2026-09-02 by driving one live cut-off turn with the quiet gate temporarily at 0 — margin -28.39 → continue, and she resumed mid-sentence. Gate: `G-CONTINUATION-MARGIN`; the knob's own both-ways legs are in `G-TUNING`. |
...
| **Why engine stays default** | the model's own vision tower is the one that knows her room; the VL door is the cheaper eye for the hourly look when the GPU is busy — the operator's call, measured look by look |
