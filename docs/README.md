...
| [`LANES.md`](LANES.md) | reference | **LIVE** | **THE six ways a fact reaches her**, and why each exists: the cached prefix (KV token 0 — stale or a re-prefill, no third outcome), the per-turn system row and its idempotence sentinel, the staple on his turn and the measurement that says never to put a fact about HER there, the tool loop, the kairos nudge, and the overnight growth loop. Read before adding anything to her context. |
| [`PANELS.md`](PANELS.md) | reference | **LIVE** | Every window in the room: the route it reads, whether it writes, and whose it is. Journal and Her own time have no POST behind them by construction — a companion whose diary the operator can edit does not have a diary. |
| [`TELEMETRY.md`](TELEMETRY.md) | reference | **LIVE** | His body as a source she can be present to: the watch agent, the ingest door, the store, the seam that decides what she reads, and the panel. |
| [`HOME-ASSISTANT.md`](HOME-ASSISTANT.md) | reference | **LIVE** | The house as a second door into the same telemetry lane: the HA bridge, the entity map (what crosses and what it becomes), the on-request hands and their allowlist, and the House window (the beachhead). *(This cell was a copy-paste of TELEMETRY's for its whole life — 2026-08-29 audit.)* |
| [`PRIMING.md`](PRIMING.md) | guide | LIVE (2026-08-27) | **Where priming goes and why.** A capability that ships unprimed is a capability nobody has: the tool schema says what a verb DOES, the persona says when it is HERS TO REACH FOR. Measured at 67 of 82 tools unprimed. Held by G-PRIMING. |
...
| [`ANON-MODE.md`](ANON-MODE.md) | reference | **LIVE** | **Off the record** (2026-08-23, the operator's ask): the taskbar switch that keeps her entirely herself and writes nothing down. The nineteen doors in `anon.DOORS` (fourteen write, five egress, count checked 2026-08-29) and where each is guarded, the third door class that is neither — her PROMPT, which the room kept re-sending after the switch went off, the two deliberate exceptions, the persona shadow, the line she is told, and the table of what it does NOT touch. Volatile by design — a restart ends it. `g_anon.py` diffs the disk. |
