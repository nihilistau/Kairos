...
import { Button, Chip, State } from '../kit/parts.jsx'
...
export default function Stage() {
  ...
  return <div className="pad stg"><Body state={s}>{d => <StageView d={d} act={act} />}</Body></div>
}

/* The window's body, split out so G-ROOM-KIT leg 11 can render it with a fixture. The
 * status row was a .chips row of three statuses and one action: the statuses are chips
 * now (their titles verbatim), and STOP is a danger Button — still rendered whenever a
 * scene is live, still one click, still no argument. */
export function StageView({ d, act }) {
  if (d.ok === false) return <State kind="error">{'stage unavailable — ' + d.error}</State>
  const sc = d.scene
  const cap = d.max_heat ?? 7
  return (
    <>
      <div className="stg-bar">
        <Chip tone={d.enabled ? 'ok' : 'neutral'} dot={d.enabled}>{d.enabled ? 'armed' : 'switched off'}</Chip>
        <Chip title="the operator's hard ceiling — the scene never goes past it">
          {'ceiling ' + (d.ladder?.[cap]?.name || cap)}
        </Chip>
        <Chip title="the pacing dial: >1 makes the build slower">{'pacing ×' + (d.dwell_scale ?? 1)}</Chip>
        {sc ? (
          <Button variant="danger" size="sm" className="stg-stop" onClick={() => act({ op: 'exit' })}
                  title="drops character immediately, from any rung">■ stop the scene</Button>
        ) : null}
      </div>
        ...
        <>
          <div className="stg-live" style={{ '--h': RUNG_HUE(sc.level) }}>
            ...
            <div className="stg-nums">
              ...
              {sc.opened ? null : <Chip tone="warn">opening not yet delivered</Chip>}
            </div>
          </div>
            ...
            {(d.deck || []).map(c => (
              <div key={c.id} className="stg-card">
                <div className="stg-ct">
                  ...
                  {c.authored ? <Chip tone="ok" title="authored: dropped into var/room/scenarios/">authored</Chip> : null}
                </div>
                <div className="muted">{c.premise}</div>
                <Button size="sm" className="stg-begin" onClick={() => act({ op: 'enter', id: c.id })}>begin</Button>
              </div>
            ))}
          </div>
        </>
