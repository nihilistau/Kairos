...
function Mark({ state, on = 'ready', off = 'not set', warn = 'needs attention' }) {
  // a kit Chip since redesign stage 4: `su-mark good|warn|bad` reached colour only
  // through .su-mark.*, and its warn was his gold — "needs attention" is the kit's warn
  const tone = state === true ? 'ok' : state === 'warn' ? 'warn' : 'neutral'
  return <Chip tone={tone}>{state === true ? on : state === 'warn' ? warn : off}</Chip>
}
...
export default function Setup() {
  ...
  return (
    <div className="su pad">
      <Body state={state}>{d => <SetupView d={d} />}</Body>
    </div>
  )
}
