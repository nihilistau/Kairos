...
export function LibrariansView({ d, onRebuild }) {
  const line = !d.armed
    ? <Chip wrap>off — SP_AUX is off in the profile; every caller keeps its pre-aux behaviour</Chip>
  ...
  return (
    <>
      ...
      <div className="lib-row">
        ...
        {' '}<Button size="sm" onClick={onRebuild}>rebuild index</Button>
      </div>
...
export default function Librarians() {
  ...
  return (
    <div className="pad">
      <Body state={s}>{d => <LibrariansView d={d} onRebuild={() => api.auxRebuild().then(s.refresh)} />}</Body>
    </div>
  )
}
