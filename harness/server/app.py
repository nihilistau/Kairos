...
# ── THE DAY BOUNDARY LIVES IN day.py (2026-09-11, Stage 4 of the split) ───────────
# The extraction the Stage 3 receipt named as owed: turn.py's `_settle_turn` reached back
# across a module edge for `_append_day_turn`, and that shim points at day.py now.
# Re-exported here because this file still calls into the boundary (`/v1/start` seeds from
# it, the restart route and the chat handler read the transcript helpers) and because the
# gates import these out of `app` — `g_day_transcript`, `g_consolidate`, `g_own_time`,
# `g_synthetic_quarantine`. A re-export keeps every one of those working while the
# definitions live somewhere a person can find.
#
# `start_ha_poller` deliberately did NOT go: it sat in the middle of that region of the
# file and shares nothing with the boundary but its neighbourhood. Adjacency is not a seam.
from harness.server import day as _day  # noqa: E402
_CONSOLIDATE_STATE = _day._CONSOLIDATE_STATE
_consolidate_marker = _day._consolidate_marker
_consolidate_last_day = _day._consolidate_last_day
_consolidate_mark = _day._consolidate_mark
_day_transcript_path = _day._day_transcript_path
_append_day_turn = _day._append_day_turn
_seed_kairos_from_day = _day._seed_kairos_from_day
_read_day_transcript = _day._read_day_transcript
_recent_transcript = _day._recent_transcript
_chat_from_rows = _day._chat_from_rows
_longest_session = _day._longest_session
_from_his_last_turn = _day._from_his_last_turn
_continuable_history = _day._continuable_history
_reseed_own_time_canon = _day._reseed_own_time_canon
_narratable = _day._narratable
_longest_transcript = _day._longest_transcript
run_consolidation = _day.run_consolidation
_day_key = _day._day_key
_quiet_for = _day._quiet_for
start_consolidation_ticker = _day.start_consolidation_ticker
