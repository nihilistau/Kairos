...
check("she wears what she picked, full stop", r["shown"] == "t3", r)
check("...and nothing claims she was held", r["clamped"] is False, r)
check("HER CHOICE IS KEPT, not rewritten to what he allows", r["hers"] == "t3", r)
...
WD.choose(tier="t0", by="her")
check("her choice stands whatever a scene is doing", WD.resolve()["shown"] == "t0")
WD.choose(tier="t2", by="her")
check("what she chose is what is shown", WD.resolve()["shown"] == "t2")
...
WD.import_clip(os.path.join(SB, "see-thru-silver-nightie-bedroom-laydown-hands-30s.mp4"), tier="t2")
WD.import_clip(os.path.join(SB, "desk-mesh-top-talking.mp4"), tier="t0")
allc = WD.clips()
...
check("the everyday outfit is not framed as a lesser one",
      "default" in WD.OUTFITS["t0"]["about"])
...
WD.choose(tier="t1", clip="desk-mesh-top-talking", by="her")
...
r = WD.request("by the window in morning light", tier="t2", by="her")
...
check("...and carries the tier's own wardrobe", WD.TIER_WORDS["t2"]["wearing"] in pr)
...
r2 = WD.request("something he never made", tier="t0")
...
WD.choose(tier="t2", look=r["id"], by="her")
check("both are held at once",
      WD.current()["look"] == r["id"] and WD.current()["tier"] == "t2", WD.current())
WD.choose(tier="t1")
...
WD.choose(tier=r["tier"], look=r["id"], by="her")
f = WD.favourites()
...
n_before = len([w for w in WD.worn_log() if w["what"] == "t1"])
WD.choose(tier="t1", look="", by="him")
rows = [w for w in WD.worn_log() if w["what"] == "t1"]
check("HE dressing her is recorded, once, as his", len(rows) == n_before + 1
      and rows[-1]["by"] == "him", rows[-3:])
WD.praise("t0", "you look good like that")
f2 = WD.favourites()
top = f2[0] if f2 else {}
check("HIS WORD OUTRANKS HER HABIT — one praise beats two wearings",
      top.get("id") == "t0", [(x["id"], x["score"]) for x in f2])
...
r3 = WD.request("something new", tier="t0")
...
WD.choose(tier="t0", look=r3["id"], by="her")
...
g = WD.request("laughing properly, head tipping back", tier="t2", by="her", kind="gesture")
...
check("a look is still composed wardrobe-first",
      "Wardrobe:" in WD.compose_prompt("something", "t2", "look"))
...
check("a mark SAID moves her", WD.current()["tier"] == "t2", WD.current())
...
check("REPLAYING it does not redress her", WD.current()["tier"] == _worn_then["tier"],
...
check("the NIGHTSHIFT curator cannot dress her out of yesterday's transcript",
      WD.current()["tier"] == _worn_then["tier"], WD.current())
...
_lk = WD.request("a grey wool coat, the collar up", tier="t0")
os.makedirs(os.path.join(SB, "looks"), exist_ok=True)
...
WD.choose(tier="t2", look="", by="her")
_n = WD.wearing_now()
check("with nothing on over it, she is wearing the OUTFIT",
      _n["kind"] == "outfit" and _n["words"] == WD.OUTFITS["t2"]["wearing"], _n)

WD.choose(tier="t2", look=_lk["id"], by="her")
_n = WD.wearing_now()
...
check("...and the outfit underneath is still named, just not as the headline",
      WD.OUTFITS["t2"]["wearing"] in WD.describe(), WD.describe()[:200])
...
_cl = WD.import_clip(os.path.join(SB, "desk-mesh-top-talking.mp4"), tier="t0")
...
WD.choose(tier="t3", look="", clip="", by="her")
_wn = WD.wearing_note()
